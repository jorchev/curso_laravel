<x-layouts.app title="Login" meta-description="Contact meta description">
    <h1 class="my-4 text-3xl text-center text-sky-500 dark:text-sky-500 font-bold">Login</h1>
    ...
    <form class="max-w-xl px-8 py-4 mx-auto bg-white rounded shadow dark:bg-slate-800" action="{{ route('login') }}" method="POST">
        ...
        <div class="space-y-4">
            <label class="flex items-center">
                <input class="border rounded dark:bg-slate-900 dark:border-slate-800 border-slate-300 dark:border-slate-700 text-sky-700 focus:border-sky-300 focus:ring focus:ring-slate-300 dark:focus:ring-slate-800 focus:ring-opacity-50" type="checkbox" name="remember">
                <span class="cursor-pointer ml-2 text-slate-600 dark:text-slate-400">Remember me</span>
            </label>
        </div>
